...
export function Sidenav({ brandName, routes, userRole }) {
  ...
  userRole = user_Role; 
  const [userData, setUserData] = useState({ UserDetails: {}, Branch: {}, UserRole: {} });
  ...
  return (
     <aside
      ...
      } fixed inset-0 z-50 my-4 ml-4 h-[calc(100vh-32px)] w-72 max-h-[100%] rounded-xl transition-transform duration-300 xl:translate-x-0 overflow-y-auto`}
    >
      <div
    ...
    {pages.map(({ icon, name, path, children }) => (
      <li key={name}>
        {children ? (
          userRole === 1 && (
            ...
            >
              {icon}
              <Typography
                color={sidenavType === "dark" ? "white" : "blue-gray-200"}
          ...
          (userRole === 1 && !["Help"].includes(name) || (userRole === 2 && !["Setup", "Help"].includes(name)) || (userRole !== 1 && userRole !== 2 && !["Setup"].includes(name))) ? (
        ...
        <li className="list-none">
          <button onClick={handleLogout} className="flex items-center gap-4 px-4 capitalize">
